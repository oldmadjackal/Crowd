...
/*		    	Программный модуль                           */
     ...
     static   Crowd_Module_Show  ProgramModule ;
...

{
 ...
 F_SHOW_API Crowd_Kernel *GetEntry(void)
...
/*                            Список команд                          */
  ...
  struct Crowd_Module_Show_instr  Crowd_Module_Show_InstrList[]={

 { "help",    "?", "#HELP   - список доступных команд", 
                    NULL,
                   &Crowd_Module_Show::cHelp       },
 { "color",   "c", "#COLOR   - установить цвет объекта", 
                   ...
                   &Crowd_Module_Show::cColor      },
 { "visible", "v", "#VISIBLE - вкл./выкл. видимость объекта", 
                   " VISIBLE <имя> 0\n"
                   "   Скрыть объект\n"
                   " VISIBLE <имя> 1\n"
                   "   Показать объект\n",
                   &Crowd_Module_Show::cVisible    },
 {  NULL }
                                                            } ;
...
/*		     Общие члены класса             		     */
    ...
    struct Crowd_Module_Show_instr *Crowd_Module_Show::mInstrList=NULL ;

/*********************************************************************/
/*								     */
/*		       Конструктор класса			     */

     Crowd_Module_Show::Crowd_Module_Show(void)

{
    ...
    identification="Show" ;
        ...
        mInstrList=Crowd_Module_Show_InstrList ;
}
...
/*		        Получить параметр       		    */
     ...
     int  Crowd_Module_Show::vGetParameter(char *name, char *value)
...
/*		        Стартовая разводка                           */
    ...
    void  Crowd_Module_Show::vStart(void)

{
/*-------------------------------------------- Регистрируем свойство */

   feature_modules=(Crowd_Kernel **)
...
/*		        Создать свойство                	     */
    ...
    Crowd_Feature *Crowd_Module_Show::vCreateFeature(Crowd_Object *object, Crowd_Feature *feature_ext)

{
  Crowd_Feature *feature ;


    if(feature_ext!=NULL)  feature= feature_ext ;
    else                   feature=new Crowd_Feature_Show ;
...
/*                    Отобразить связанные данные                   */
    ...
    void  Crowd_Module_Show::vShow(char *layer)
...
/*		        Выполнить команду       		     */
  ...
  int  Crowd_Module_Show::vExecuteCmd(const char *cmd)
...
/*		        Записать данные в строку		    */
    ...
    void  Crowd_Module_Show::vWriteSave(std::string *text)
...
/*		      Реализация инструкции Help                    */
  ...
  int  Crowd_Module_Show::cHelp(char *cmd)
...
/*       COLOR  <Имя> RGB <R-индекс>:<G-индекс>:<B-индекс>          */
  ...
  int  Crowd_Module_Show::cColor(char *cmd)
...
/*     VISIBLE  <Имя> 1                                             */
  ...
  int  Crowd_Module_Show::cVisible(char *cmd)
...
/*---------------------------------- Подготовка отображения объектов */
    ...
    for(j=0 ; j<OBJECTS[i]->Features_cnt ; j++)
      if(!stricmp(OBJECTS[i]->Features[j]->Type, "Show"))  
            ((Crowd_Feature_Show *)OBJECTS[i]->Features[j])->Visible=visible ;
...
/*                   Отображение сцены                              */
  ...
  int  Crowd_Module_Show::iShowScene(void)
...
/*---------------------------------- Подготовка отображения объектов */
    ...
    while(status==0) {
       ...
       for(i=0 ; i<OBJECTS_CNT ; i++)
        for(j=0 ; j<OBJECTS[i]->Features_cnt ; j++)
          if(!stricmp(OBJECTS[i]->Features[j]->Type, "Show"))  
               ((Crowd_Feature_Show *)OBJECTS[i]->Features[j])->Show() ;
...
/*		       Конструктор класса			    */
     ...
     Crowd_Feature_Show::Crowd_Feature_Show(void)
...
/*		        Считать данные из строки		    */
    ...
    void  Crowd_Feature_Show::vReadSave(       char *title,  
                                        std::string *data, 
                                               char *reference)
...
/*------------------------------------------------------------- Тело */
          ...
          if(!memicmp(work, "$BODY", strlen("$BODY"))) {
               ...
               Bodies=(Crowd_Feature_Show_Body *)
...
/*- - - - - - - - - - - - - - - - - - - - -  Расширение списка точек */
           if(num>=VERTEXES_CNT) {
               ...
               VERTEXES=(Crowd_Feature_Show_Vertex *)
...
/*- - - - - - - - - - - - - - - - - - - - - - Добавление новой грани */
              FACETS=(Crowd_Feature_Show_Facet *)
...
/*                     Выдать информацию о свойстве   		    */
    ...
    void Crowd_Feature_Show::vGetInfo(std::string *text)
...
/*                     Работа с параметрами           		    */
     ...
     int Crowd_Feature_Show::vParameter(char *name, char *action, char *value)
...
/*                          Удалить тело                             */
    ...
    void Crowd_Feature_Show::vBodyDelete(char *body)
{
   int  cnt ;
   int  n_body ;
...
/*---------------------------------------------------- Удаление тела */
       ...
       if(this->Bodies[n_body].list_idx)                            /* Освобождаем дисплейный список */                                       
           Crowd_Kernel::display.ReleaseList(this->Bodies[n_body].list_idx) ;
...
/*                   Задание базовой точки тела                     */
    ...
    void Crowd_Feature_Show::vBodyBasePoint(  char *body,                   
                                            double  x, 
                                            double  y, 
                                            double  z )
{
...
/*                     Задание ориентации тела                      */
    ...
    void Crowd_Feature_Show::vBodyAngles(  char *body,                   
                                         double  azim, 
                                         double  elev, 
                                         double  roll    )
{
...
/*               Задание матрицы положения тела                     */
    ...
    void Crowd_Feature_Show::vBodyMatrix(  char *body,
                                         double  matrix[4][4] )
{
   int  n_body ;
...
/*           Задание смещения положения и ориентации тела           */
    ...
    void Crowd_Feature_Show::vBodyShifts(   char *body,                   
                                         double  x, 
                                         double  y, 
                                         double  z, 
                                         double  azim, 
...
/*              Задание списка параметров тела                      */
    ...
    void Crowd_Feature_Show::vBodyPars(char *body, 
                                        struct Crowd_Parameter *pars)
{
   int  n_body ;
...
/*                       Показать объект в сцене                    */
    ...
    void  Crowd_Feature_Show::Show(void)

{
              Crowd_Kernel *Calc ;
 Crowd_Feature_Show_Vertex  n_vector ;
                  GLdouble  matrix[16] ;
                       int  i ;
                       int  j ;
                       int  k ;
     ...
     if(this->Visible==0) {
      ...
      for(i=0 ; i<this->Bodies_cnt ; i++)                           /* Освобождаем дисплейные списки */
        if(this->Bodies[i].list_idx) {                              /*   всех тел объекта            */
            Crowd_Kernel::display.ReleaseList(this->Bodies[i].list_idx) ;
...
/*- - - - - - - - - - - - - - - -  Резервирование дисплейного списка */
     if(this->Bodies[i].list_idx==0) {
           ...
           this->Bodies[i].list_idx=Crowd_Kernel::display.GetList(0) ;
...
/*                     Вычислить нормаль к плоскости                */
  ...
  void  Crowd_Feature_Show::iCalcNormal(Crowd_Feature_Show_Vertex *point_0,
                                        Crowd_Feature_Show_Vertex *point_1,
                                        Crowd_Feature_Show_Vertex *point_2,
                                        Crowd_Feature_Show_Vertex *normal  )
...
/*            Определение нужного вычислителя                       */
  ...
  Crowd_Kernel *Crowd_Feature_Show::iGetCalculator(void)
