...
/********************************************************************/
/**                                                                **/
/**	 ОПИСАНИЕ КЛАССА МОДУЛЯ УПРАВЛЕНИЯ ОБЪЕКТОМ "ОПЕРАТОР ОПРОСОВ" **/
/**                                                                **/
/********************************************************************/
/********************************************************************/
...
/*--------------------------------------------------- Проверка имени */
    ...
    if(data->name[0]==0) {                                           /* Если имя НЕ задано */
              SEND_ERROR("Секция SURVEY: Не задано имя объекта") ;
                                return(NULL) ;
                         }

       for(i=0 ; i<OBJECTS_CNT ; i++)
         if(!stricmp(OBJECTS[i]->Name, data->name)) {
              SEND_ERROR("Секция SURVEY: Объект с таким именем уже существует") ;
                                return(NULL) ;
                                                    }
...
/*- - - - - - - - - - - - Если модель задана названием и библиотекой */
   if(data->path[0]==0) {
    ...
    if(data->model[0]==0) {                                         /* Если модель НЕ задано */
              SEND_ERROR("Секция SURVEY: Не задана модель объекта") ;
                                return(NULL) ;
                          }
        ...
        for(end=models_list ; *end ; ) {                            /* Ищем модель по списку */
           ...
           if(*end==0) {
              SEND_ERROR("Секция SURVEY: Неизвестная модель тела") ;
                                return(NULL) ;
                       }
       ...
       if((data->pars[i].text [0]==0 &&
          ...
          (data->pars[i].text [0]!=0 &&
           data->pars[i].value[0]==0   )   ) {
              ...
              SEND_ERROR("Секция SURVEY: Несоответствие числа параметров модели") ;
                                return(NULL) ;
                                             }
/*------------------------------------------------- Создание обьекта */
    ...
    if(object==NULL) {
              SEND_ERROR("Секция SURVEY: Недостаточно памяти для создания объекта") ;
                        return(NULL) ;
                     }
...
/*- - - - - - - - - - - - - - - - - - - - - - - -  Заносим параметры */
     for(i=0 ; i<5 ; i++)
       if(data->pars[i].text[0]!=0) {
        ...
        if(PAR==NULL) {
                         SEND_ERROR("Секция SURVEY: Переполнение памяти") ;
                                            return(NULL) ;
                      }
...
/*-------------------------------------------------- Описание модуля */
    ...
    if(!stricmp(name, "$$MODULE_NAME")) {
         ...
         sprintf(value, "%-20.20s -  Оператор опросов (общая модель)", identification) ;
                                        }
/*-------------------------------------------------------------------*/
...
/*--------------------------------------------- Выделение инструкции */
   ...
   for(i=0 ; mInstrList[i].name_full!=NULL ; i++)                   /* Ищем команду в списке */
     ...
     if(mInstrList[i].name_full==NULL) {                            /* Если такой команды нет... */
       ...
       if(status)  SEND_ERROR("Секция SURVEY: Неизвестная команда") ;
                                            return(-1) ;
                                       }
...
/********************************************************************/
/*                                                                  */
/*                Поиск обьекта типа SURVEY по имени                */
...
/*-------------------------------------------- Контроль типа объекта */ 
     ...
     if(strcmp(OBJECTS[i]->Type, "Survey")) {
           ...
           SEND_ERROR("Объект не является объектом типа SURVEY") ;
                            return(NULL) ;
                                            }
/*-------------------------------------------------------------------*/ 
     ...
     int  Crowd_Object_Survey::vEvent(long  t, char *type, void  *data, Crowd_Kernel *task)
{
   char  error[1024] ;
     ...
     if(!stricmp(this->behavior_model, "$DEFAULT"))   this->iBehaviorDefault(t, type, data, task) ;
                  ...
                  sprintf(error, "Section SURVEY: Неизвестный встроенный сценарий поведения: %s", this->behavior_model) ;
               SEND_ERROR(error) ;
                     return(-1) ;
                                                    }
