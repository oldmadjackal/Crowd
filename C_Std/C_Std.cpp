...
/*		    	Программный модуль                          */
     ...
     static   Crowd_Calc_Std  ProgramModule ;
...

{
 ...
 C_STD_API Crowd_Kernel *GetEntry(void)
...
/*		    	Буфер текста                                */
   ...
   static  char *Crowd_Calc_Std_text    =NULL ;
   static  char *Crowd_Calc_Std_text_cur=NULL ;
...
/*		       Конструктор класса			    */
     ...
     Crowd_Calc_Std::Crowd_Calc_Std(void)
...
/*		        Получить параметр       		    */
     ...
     int  Crowd_Calc_Std::vGetParameter(char *name, char *value)
...
/*		        Стартовая разводка                          */
    ...
    void  Crowd_Calc_Std::vStart(void)
...
/*		        Вычислить выражение     		    */
  ...
  int  Crowd_Calc_Std::vCalculate(            char  *expression_type,
                                              char  *expression, 
...
{
  Crowd_Calc_Std_context *context ;
                     int  const_flag ;
                     int  exec_only ;
                     int  i ;
                     int  j ;
...
//
//            lng_debug     = 1 ;
//            lng_user_debug=Crowd_Calc_Std_debug ;

/*------------------------------------------------ Очистка контекста */

   if(!stricmp(expression_type, "CLEAR")) {

          context=(Crowd_Calc_Std_context *)*context_ptr ;
...
/*--------------------------------------------- Извлечение контекста */
       ...
       if(context==NULL) {
            ...
            context=(Crowd_Calc_Std_context *)
...
/*- - - - - - - - - - - - - - - - - Подготовка текста для компиляция */
     if(!stricmp(expression_type, "STD_EXPRESSION")) {
                ...
                Crowd_Calc_Std_text_norm(expression) ;

            Crowd_Calc_Std_text=(char *)calloc(1, strlen(expression)+1000) ;
         if(Crowd_Calc_Std_text==NULL) {
             SEND_ERROR("Вычислитель> Недостаточно памяти для текста") ;
                                                  return(-1) ;
                                       }

            sprintf(Crowd_Calc_Std_text, "ZZZ=%s\n"
                                       "RETURN(ZZZ)", expression) ; 
                                                     }
     else                                            {
            Crowd_Calc_Std_text=strdup(expression) ;
         if(Crowd_Calc_Std_text==NULL) {
             SEND_ERROR("Вычислитель> Недостаточно памяти для текста") ;
                                                  return(-1) ;
                                     }
                        ...
                        Crowd_Calc_Std_text_cur=NULL ;
             C.get_text=Crowd_Calc_Std_get_text ;                   /* Уст.п/п построчной передачи текста */
					                            /*         NULL - конец текста        */
/*- - - - - - - - - - - - - - - - - - - - - -  Собственно компиляция */
                L_compile(&context->module_decl) ;

                    free(Crowd_Calc_Std_text) ;
/*- - - - - - - - - - - - - - - - - - - - - - - - - Обработка ошибок */
     if(!stricmp(expression_type, "STD_EXPRESSION")) {

      if(lng_error)  sprintf(error, "Error: %s \nBad:<%s>",
		 	          Crowd_Calc_Std_error(lng_error), lng_bad) ;
...
/*                   Отладочная печать в файла                      */
   ...
   void  Crowd_Calc_Std::iDebug(char *text, char *path,  int  condition)
{
    if(condition)  iDebug(text, path) ;
}


   void  Crowd_Calc_Std::iDebug(char *text, char *path)
{
...
/*                 Нормализация строки выражения                    */
  ...
  void  Crowd_Calc_Std_text_norm(char *text)
...

{
  ...
  if(Crowd_Calc_Std_text_cur==NULL)  
          Crowd_Calc_Std_text_cur=Crowd_Calc_Std_text ;
  else    Crowd_Calc_Std_text_cur=  next ;

  if(Crowd_Calc_Std_text_cur==NULL)  return(NULL) ;

          next=strchr(Crowd_Calc_Std_text_cur, '\n') ;
       if(next!=NULL) {
                         *next=0 ;
                          next++ ;
                      }
   
  return(Crowd_Calc_Std_text_cur) ;
}
...
/*                    Выдача текста предупреждения по коду           */
  ...
  char *Crowd_Calc_Std_error(int  code) 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  char *Crowd_Calc_Std_warning(int  code) 
...
/*                     Процедура отладки вычислителя                 */
  ...
  void  Crowd_Calc_Std_debug(int  frame)

{
   Crowd_Calc_Std  module ;
             char  text[1024] ;
