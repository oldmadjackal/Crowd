...
/* CrowdRoot.cpp */
      int  CrowdRoot_system           (void) ;                  /* Обработка системных сообщений */
      int  CrowdRoot_error            (char *, int) ;           /* Обработка сообщений об ошибках */
      int  CrowdRoot_command_start    (char *) ;                /* Запуск команд */
      int  CrowdRoot_command_processor(char *) ;                /* Процессор команд */
      int  CrowdRoot_command_read     (char **) ;               /* Обработчик командного файла */
      int  CrowdRoot_command_write    (char *) ;                /* Запись командного файла */
     void  CrowdRoot_command_normalise(char *) ;                /* Нормализация командной строки */
