/*********************************************************************/
/*                                                                   */
/*                СИСТЕМА МОДЕЛИРОВАНИЯ СЕТЕЙ АГЕНТОВ                */
/*                                                                   */
/*********************************************************************/
...
/*------------------------------------ Обработчики элементов диалога */
  ...
  static union WndProc_par  Tmp_WndProc ;
   ...
   LRESULT CALLBACK  CrowdRoot_Cmd_WndProc(HWND, UINT, WPARAM, LPARAM) ;
...
/*---------------------------------------- Подгрузка рабочих модулей */
	    ...
	    Kernel.SetKernel(NULL, hInstance) ;
         ...
         if(Kernel.modules_cnt==0)
                      CrowdRoot_error("No active modules loaded", MB_ICONERROR) ;
...
/*------------------------------- Регистрация класса первичного окна */
      ...
      sprintf(ClassName, "CrowdRoot_Frame_%d", _getpid()) ;

	FrameWindow.lpszClassName= ClassName ;
	FrameWindow.hInstance    = hInstance ;
	FrameWindow.lpfnWndProc  = CrowdRoot_window_processor ;
    ...
    if(!RegisterClass(&FrameWindow)) {
                 sprintf(text, "CrowdRoot_Frame register error %d", GetLastError()) ;
         CrowdRoot_error(text, MB_ICONERROR) ;
	                                return(-1) ;
				     }
/*----------------------------------------- Создание первичного окна */

    hFrameWindow=CreateWindow(ClassName, "CrowdRoot", 
...
/*---------------------------------------- Создание диалогового окна */
        ...
        hDialog=CreateDialog(hInst, "IDD_MAIN",
		                  hFrameWindow, CrowdRoot_main_dialog) ;
...
/*		Обработчик сообщений "рамочного" окна		     */
 ...
 LRESULT CALLBACK  CrowdRoot_window_processor(  HWND  hWnd,     UINT  Msg,
...
/*- - - - - - - - - - - - - - - - -  Инициализация значеий элементов */
            SETs(IDC_COMMAND, "@Tests\\Spectr.crowd") ;
...
/*- - - - - - - - - - - - - - - - - "Внешнее" редактирование задания */
        if(wParam==_USER_EXECUTE_COMMAND) {
            ...
            if(end!=NULL)  strcpy(end, end+1) ;
                ...
                CrowdRoot_command_start(command) ;
...
/*- - - - - - - - - - - - - - - - - - - - - - -  Сообщение об ошибке */
        if(wParam==_USER_ERROR_MESSAGE) {
                    ...
                    CrowdRoot_error((char *)lParam, MB_ICONERROR) ;
                			      return(FALSE) ;
                                        }
/*- - - - - - - - - - - - - - - - - - - - - - -  Сообщение об ошибке */
        if(wParam==_USER_INFO_MESSAGE) {

                      CrowdRoot_error((char *)lParam, MB_ICONINFORMATION) ;
                			      return(FALSE) ;
                                       }
/*- - - - - - - - - - - - - - - - - - - - - - -  Отображение команды */
...
/*	          Обработка сообщений об ошибках                     */    
   ...
   int  CrowdRoot_error(char *message, int  icon)
...
/*                       Запуск команд                              */
   ...
   int  CrowdRoot_command_start(char *cmd)
...
/*------------------------------------------------------- Подготовка */
        ...
        CrowdRoot_threads("ADD", command) ;                         /* Регистрируем рабочий поток */

/*------------------------------------------------ Выпонение команды */

                CrowdRoot_command_processor(command) ;

/*------------------------------------------------------- Завершение */

     if(WaitCommandEnd)  SetEvent(hCommandEnd) ;

              CrowdRoot_threads("DELETE", NULL) ;                   /* Исключаем рабочий поток */
...
/*                    Процессор команд                              */
   ...
   int  CrowdRoot_command_processor(char *command)
...
/*-------------------------------------------- Цикл командных файлов */
  ...
  do {                                                              /* CIRCLE.0 */
        ...
        if(file_rd_flag) {                                          /* Считывание команды из файла */
                            status=CrowdRoot_command_read(&command) ;
              ...
              CrowdRoot_command_normalise(command) ;                /* Нормализация команды */

        if(file_wr_flag) {                                          /* Запись команды в файл */
                            CrowdRoot_command_write(command) ;
                         }
...
#define  CP  commands_pool
        ...
        for(i=0 ; CP[i] ; i++)
          ...
          if(CP[i]==NULL) {
                CP[i]=(CrowdRoot_commands *)calloc(1, sizeof(*CP[i])) ;
                          }
...
//                               break ;  }
      ...
      if( next!=NULL) *next=  0 ;
          ...
          CrowdRoot_command_normalise(cmd_buff) ;
...
/*                 Обработчик командного файла                      */
   ...
   int  CrowdRoot_command_read(char **command)
...
/*--------------------------------- Предварительная обработка строки */
                ...
                CrowdRoot_command_normalise(buff) ;                 /* Нормализация строки */
...
/*                   Запись командного файла                        */
   ...
   int  CrowdRoot_command_write(char *command)
...
/*                 Нормализация командной строки                    */
  ...
  void  CrowdRoot_command_normalise(char *command)
...
/*-------------------------------------- Получение параметров камеры */
       ...
       status=CrowdRoot_look("Main", "Get", &look_x, &look_y, &look_z,
...
/*-------------------------------------- Установка параметров камеры */
       ...
       status=CrowdRoot_look("Main", "Set", &look_x, &look_y, &look_z,
                                               NULL,    NULL,    NULL ) ;
    if(status) {
                    return(-1) ;
               }
/*------------------------------------------------------ Перерисовка */
    ...
    if(quiet_flag==0)  CrowdRoot_redraw("Main") ;
...
/*-------------------------------------- Получение параметров камеры */
       ...
       status=CrowdRoot_look("Main", "Get",    NULL,    NULL,    NULL, 
                                            &look_a, &look_e, &look_r ) ;
    if(status) {
                   SEND_ERROR("Неизвестное окно") ;
                      return(-1) ;
...
/*-------------------------------------- Установка параметров камеры */
       ...
       status=CrowdRoot_look("Main", "Set",    NULL,    NULL,    NULL, 
                                            &look_a, &look_e, &look_r ) ;
    if(status) {
                    return(-1) ;
               }
...
/*-------------------------------------- Получение параметров камеры */
       ...
       status=CrowdRoot_zoom("Main", "Get", &zoom) ;
    if(status) {
                   SEND_ERROR("Неизвестное окно") ;
                      return(-1) ;
               }
...
/*-------------------------------------- Установка параметров камеры */
       ...
       status=CrowdRoot_zoom("Main", "Set", &zoom) ;
    if(status) {
                    return(-1) ;
               }
/*------------------------------------------------------ Перерисовка */
...
/*------------------------------------------------ Извлечение данных */
    ...
    if(status) {
              ...
              CrowdRoot_zoom  ("Main", "Get", &zoom) ;
              CrowdRoot_lookat("Main", "Get",  look_at) ;
    ...
    if(path[0]==0) {
	   ...
	   memset(&file_choice, 0, sizeof(file_choice))  ;
		   ...
		   file_choice.Flags       = OFN_ENABLEHOOK   |
                                             OFN_FILEMUSTEXIST ;
		   file_choice.lpfnHook    = iCrowdRoot_read_HookProc ;
